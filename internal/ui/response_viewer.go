...
func (s ResponseViewerStyle) Layout(gtx layout.Context) layout.Dimensions {
	...
	if s.Wrap != v.chunkHeightsWrap || (s.Wrap && v.chunkHeightsWidth != size.X) {
		v.invalidateChunkHeights()
		v.chunkHeightsWrap = s.Wrap
		v.chunkHeightsWidth = size.X
		v.maxLineWidth = 0
		v.scrollX = 0
	}
	v.padChunkHeights()
	...
	for i, h := range v.chunkHeights {
		if h > 0 {
			...
			totalH += v.estimateChunkHeight(i, exactLineH, charAdv, size.X, s.Wrap)
		}
	}
	if totalH < size.Y {
		totalH = size.Y
	...
	firstLine, accumY := v.firstChunkAtFn(v.scrollY, exactLineH, charAdv, size.X, s.Wrap)
	...
	for {
		...
		off := v.coordToByteOffset(ev.Position.X, ev.Position.Y, charAdv, exactLineH, size.X, s.Wrap)
	...
	for {
		...
		case pointer.Drag:
			if v.dragActive {
				off := v.coordToByteOffset(int(ev.Position.X), int(ev.Position.Y), charAdv, exactLineH, size.X, s.Wrap)
				v.selEnd = off
				hasSel = v.selStart != v.selEnd
			}
	...
	// 3. Keyboard shortcuts when the viewer has focus. Ctrl+A selects
	//    the entire body; Ctrl+C copies the current selection (or
	//    no-ops if nothing is selected — matching native editor
	//    convention; the toolbar Copy button handles the
	//    no-selection-=>-copy-everything case).
	for {
		ev, ok := gtx.Event(
	...
	for line := firstLine; line < len(v.lineStarts); line++ {
		...
		if chunkH == 0 {
			chunkH = v.estimateChunkHeight(line, exactLineH, charAdv, size.X, s.Wrap)
		}

		// Highlight rectangle behind chunk when search match overlaps.
		if hasHL && v.highlightEnd > start && v.highlightStart < end {
			v.paintHighlight(gtx, start, end, chunkH, yOff, charAdv, s.Wrap, size.X, s.HighlightColor, v.highlightStart, v.highlightEnd)
		}
		...
		if hasSel {
			...
			if selE > start && selS < end {
				v.paintHighlight(gtx, start, end, chunkH, yOff, charAdv, s.Wrap, size.X, s.SelectionColor, selS, selE)
			}
		}
		...
		if s.Wrap {
			labelGtx.Constraints.Max.X = size.X
		} else {
