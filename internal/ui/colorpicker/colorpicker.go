...
func (p *State) IsOpen() bool { return p.Kind != KindNone }

func (p *State) Open(kind Kind, idx int, c color.NRGBA, anchor Anchor) {
	p.Kind = kind
	p.OpenIdx = idx
...
func Render(gtx layout.Context, th *material.Theme, p *State) layout.Dimensions {
	...
	paint.FillShape(gtx.Ops, theme.BorderLight, clip.UniformRRect(image.Rectangle{Min: previewMin, Max: previewMax}, 3).Op(gtx.Ops))
	...
	paint.FillShape(gtx.Ops, p.Color(), clip.UniformRRect(innerPreview, 2).Op(gtx.Ops))
	...
	material.Clickable(closeGtx, &p.CloseBtn, func(gtx layout.Context) layout.Dimensions {
		bg := theme.BgField
