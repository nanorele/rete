...
func TabFromState(ts persist.TabState) *RequestTab {
	...
	for _, fp := range ts.FormParts {
		...
		rt.FormParts = append(rt.FormParts, NewFormPart(fp.Key, fp.Value, kind, fp.FilePath, size))
	}
	for _, ue := range ts.URLEncoded {
		rt.URLEncoded = append(rt.URLEncoded, NewURLEncodedPart(ue.Key, ue.Value))
	}
...
func StateFromTab(rt *RequestTab) persist.TabState {
	reqWrap := rt.ReqWrapEnabled
	kind := TabKindHTTP
	method := rt.Method
	if rt.Method == MethodWS {
		kind = TabKindWebSocket
	}
	if rt.Method == MethodGraphQL {
		kind = TabKindGraphQL
	}
	ts := persist.TabState{
		Kind:             kind,
